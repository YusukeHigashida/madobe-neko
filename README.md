...
| ボタン | 動き |
| --- | --- |
| アプリの `⤢`（小窓の中でマウスを乗せると出る） | 元のウィンドウへ戻る |
| 小窓の `×` | 元のウィンドウごと終了する |

**押されたボタンを知る API は無い**（`×` も「タブに戻る」もどちらも小窓に
`pagehide` が飛ぶだけ）。そこでアプリ側の `⤢` を押したときだけ
`backToWindow` に印を付け、印が無ければ `×` だと判断している。
Chrome の「タブに戻る」ボタンは `⤢` と役目が重なって見分けも付かないので、
`requestWindow({ disallowReturnToOpener: true })` で消している。
これはあくまでヒントなので、効かないブラウザでは「タブに戻る」も `×` と同じ扱いになる。

なお `window.close()` で閉じられるのは**script が開いた窓か、履歴が1件のタブだけ**。
別のページから移動してきたタブは閉じられないので、そのときは元のウィンドウへ戻す。
